...
Далее под переменной `_tasks` представляющая собой `список заданий`, понимается переменная экземпляра, а под `списком заданий` и `заданиями` будут подразумеваться список словарей `List[dict]` и входящие в него словари `dict` соответственно, того же экземпляра класса. Под `экземпляром` понимается экземпляр класса.
...
- `tasks_to_get` — список индексов для получения нужных `заданий` из `списка заданий`.
...
*Возвращает:* `Tuple[int]` — список индексов `списка заданий`, соответствующие заданному фильтру. Целые числа, возвращаемые в кортеже функции, всегда **неотрицательные**.
...
*Содержимое `indexes` после сортировки:*
